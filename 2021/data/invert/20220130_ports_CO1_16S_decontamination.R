...
#Test inputs ----------------------------------------------------------------------------------------------------------------
working_directory <- here("data","invert","decontamination")
input_anacapa_path_1 <- here("data","invert","CO1_taxonomy_tables","Summary_by_percent_confidence","80","CO1_ASV_raw_taxonomy_80.txt")
input_anacapa_path_2 <- here("data","invert","Metazoa_16s_taxonomy_tables","Summary_by_percent_confidence","80","Metazoa_16s_ASV_raw_taxonomy_80.txt")
...
anacapa_table_1 %>% 
  dplyr::select(-`CO1_LA11.4.R3.S36.L001`,
         -`CO1_LA3.4.R3.S12.L001`,
         -`CO1_LB1.4.R3.S60.L001`) -> anacapa_table_1

#Generate Hash.key

colnames(anacapa_table_1) %>%
  str_remove("CO1_") -> new_colnames_1
  
colnames(anacapa_table_1) <- new_colnames_1

anacapa_table_1 %>% 
  mutate(., seq_number = str_replace(seq_number, "CO1_","CO1_")) -> anacapa_table_1

anacapa_table_1 %>% 
  dplyr::select(seq_number, sum.taxonomy) -> hash.key_1

colnames(anacapa_table_2) %>%
  str_remove("Metazoa_16s_")  %>% 
  str_remove("X16s_")-> new_colnames_2

colnames(anacapa_table_2) <- new_colnames_2

anacapa_table_2 %>% 
  mutate(., seq_number = str_replace(seq_number, "Metazoa_16s_","Metazoa_16s_")) ->anacapa_table_2
...
###Pre Occupancy Sum by Taxonomy, All PCR Tech Reps Separate Samples
  ...
  saveRDS(barcode_1_pre_df,file="Output_R/pre_occupancy_results_sum.taxonomy_tech_reps_co1_read_counts.RDS")
  write_csv(barcode_1_pre_df ,"Output_csv/pre_occupancy_results_sum.taxonomy_tech_reps_co1_read_counts.csv")
  ...
  saveRDS(barcode_2_pre_df,file="Output_R/pre_occupancy_results_sum.taxonomy_tech_reps_16s_read_counts.RDS")
  write_csv(barcode_2_pre_df ,"Output_csv/pre_occupancy_results_sum.taxonomy_tech_reps_16s_read_counts.csv")
  ...
  saveRDS(barcode_1_post_occ_df,file="Output_R/post_occupancy_results_sum.taxonomy_tech_reps_co1_read_counts.RDS")
  write_csv(barcode_1_post_occ_df ,"Output_csv/post_occupancy_results_sum.taxonomy_tech_reps_co1_read_counts.csv")
  ...
  saveRDS(barcode_2_post_occ_df,file="Output_R/post_occupancy_results_sum.taxonomy_tech_reps_16s_read_counts.RDS")
  write_csv(barcode_2_post_occ_df ,"Output_csv/post_occupancy_results_sum.taxonomy_tech_reps_16s_read_counts.csv")
...
ASV.table_long %>% 
  ...
  ASV.nested %>% 
    ...
    unnest(Step4.tibble) %>%
    filter(., !sample %in% c("LB1_0_1","LB1_0_2","LB1_0_3")) %>% 
    mutate(., barcode = case_when(str_detect(sample_name,"CO1")~"CO1",
                                 str_detect(sample_name,"16s")~"Metazoa_16s", 
                                 TRUE~"wrong")) %>% 
  ...
  holder %>% 
    group_by(barcode) %>% 
    filter(., S !="") %>% 
    dplyr::summarise(n_distinct(S))
  
  holder %>% 
    group_by(barcode) %>% 
    filter(., G !="") %>% 
    dplyr::summarise(n_distinct(G))
  
  holder %>% 
    group_by(barcode) %>% 
    filter(., `F` !="") %>% 
    dplyr::summarise(n_distinct(`F`))
  
  holder %>% 
    group_by(barcode) %>% 
    filter(., `C` !="") %>% 
    dplyr::summarise(n_distinct(`C`))
  
  
  holder %>% 
    group_by(barcode) %>% 
    filter(., `P` !="") %>% 
    dplyr::summarise(n_distinct(`P`))
  
  
  holder %>% 
    group_by(barcode) %>% 
    filter(., `D` !="") %>% 
    dplyr::summarise(n_distinct(`D`))
  ...
  calnemo_path <- here("data","invert","cal_nemo_10102019.txt")
  calnemo <- read.table(calnemo_path, header = 1, sep = "\t", stringsAsFactors = F)


intersect(holder$S,calnemo$Species)
